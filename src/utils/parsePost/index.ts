...
const metadata = [
  {
    ...
    fieldname: 'post',
    section: 'post',
    xpath: './div/div/div[2]/div/div/div[4]/div[1]/div/span/span',
  },
  {
    type: 'single',
    fieldname: 'post_photo',
    section: 'post',
    xpath: './div/div/div[2]/div/div/div[5]/div/div/button/div/div/img',
  },
  {
    type: 'single',
    fieldname: 'post_video',
    section: 'post',
    xpath: './div/div/div[2]/div/div/div[5]/div/div/div/div/video',
  },
  {
    type: 'single',
    fieldname: 'poll_name',
  ...
  },
  {
    ...
    fieldname: 'post_date',
    section: 'post',
    xpath: './div/div/div[2]/div/div/div[2]/div/div/a[2]/span/div/span/span[1]',
  },
  {
    type: 'single',
    fieldname: 'post_date',
    section: 'post',
    xpath: './div/div/div[2]/div/div/div/div[2]/div/div/a[2]/span/div/span/span[1]',
  },
  {
    type: 'single',
  ...
  return new Promise<any>((resolve) => {
    ...
    const updatePosts = async (main: HTMLElement) => {
      ...
      postLoop: for (let i = counter; i < postList.length; i++) {
        ...
        const commentBtn = postItem.querySelector('.comment-button')

        if (commentBtn instanceof HTMLButtonElement) {
          const x = commentBtn.offsetLeft
          const y = commentBtn.offsetTop
          ...
          commentBtn.dispatchEvent(event)
            ...
            switch (item.tagName) {
              ...
              default: {
                ...
                if (textContent && !post[pathItem.fieldname]) {
                  if (pathItem.fieldname === 'post_date' && compareTimeString(textContent)) {
